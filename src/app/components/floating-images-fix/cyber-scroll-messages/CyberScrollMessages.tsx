...
const CyberScrollMessages: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const updateActiveMessage = (currentVhPosition: number) => {
      if (forceAllActive) {
        setActiveIndex(0); // 全て表示モードの場合は最初のメッセージをアクティブにする
        return;
      }
      ...
      cyberMessages.forEach((msg, idx) => {
        if (currentVhPosition >= msg.position.start && currentVhPosition <= msg.position.end) {
          activeIdx = idx;
          foundActive = true;
          
          if (idx === 2 && !isFlashActive && 
              currentVhPosition >= msg.position.start && 
              currentVhPosition <= msg.position.start + 20) {
            triggerFlashEffect();
          }
        }
      });
